...
class IA3RecommendationModel(nn.Module):
...
df = pd.read_csv("helper_model/aggregated_ia3_results.csv")

derived_rows = [compute_derived_metrics_ia3(row) for _, row in df.iterrows()]
...
hyperparameter_cols = ["batch_size", "learning_rate", "epoch", "layers_tuned", "target_modules"]
...
module_options = ["q", "k", "v", "o"]
if "target_modules" in df_derived.columns:
    df_derived["target_modules"] = df_derived["target_modules"].apply(
        lambda x: eval(x) if isinstance(x, str) else (x if isinstance(x, (list, tuple)) else [])
    )
    for m in module_options:
        df_derived[f"tm_{m}"] = df_derived["target_modules"].apply(
            lambda lst: 1 if isinstance(lst, (list, tuple)) and m in lst else 0
        )
...
joblib.dump(scaler_X, "ia3_input_scaler.pkl")

scaler_y = StandardScaler()
df_derived[target_cols] = scaler_y.fit_transform(df_derived[target_cols])
joblib.dump(scaler_y, "ia3_target_scaler.pkl")

dataset = IA3Dataset(df_derived, input_cols, target_cols)
loader = DataLoader(dataset, batch_size=32, shuffle=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = IA3RecommendationModel(input_dim=len(input_cols), output_dim=len(target_cols)).to(device)
...
torch.save(model.state_dict(), "ia3_recommendation_model.pt")
print("IA3 Model trained with targets:", target_cols)
print("Input features include:", input_cols)
