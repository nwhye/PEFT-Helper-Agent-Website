...
def calculate_thresholds(
    ...
    loss_slope_col="loss_slope_mean",
    grad_norm_col="gradient_norm_mean_mean",
):
    """
    ...
    if speed_col in df.columns:
        for q in quantiles:
            thresholds[f"{speed_col}_q{int(q*100)}"] = float(df[speed_col].quantile(q))

    if loss_slope_col in df.columns:
        # NOTE: more negative is better → track lower tail
...
if __name__ == "__main__":
    input_csv = "aggregated_peft_results_exp.csv"
