...
def compute_derived_metrics(meta: dict) -> dict:
    ...
    aligned with training dynamics:
    - Training Speed
    - Loss Slope
    - Gradient Norm
    """
    ...
    df["loss_stability"] = df["train_loss_last_std"] / (df["train_loss_last_mean"].abs() + EPS)
    ...
    df["grad_stability"] = df["gradient_norm_mean_std"] / (df["gradient_norm_mean_mean"].abs() + EPS)
    df["grad_ratio"] = df["gradient_norm_mean_max"] / (df["gradient_norm_mean_min"] + EPS)

    df["eval_stability"] = df["eval_loss_std"] / (df["eval_loss_mean"].abs() + EPS)
    ...
    df["training_speed_stability"] = (
        df["training_efficiency_std"] / (df["training_efficiency_mean"].abs() + EPS)
    )

    df["gradient_norm_stability"] = (
        df["gradient_norm_mean_std"] / (df["gradient_norm_mean_mean"].abs() + EPS)
    )
